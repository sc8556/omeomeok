...
1. **데이트할 때마다 반복되는 고민** — 네이버나 카카오맵에서 맛집을 하나하나 찾고 뭘 먹을지 결정하는 시간이 너무 아까웠다. 그 시간을 줄여줄 수 있는 앱을 직접 만들어보고 싶었다.
2. **Claude Code로 바이브 코딩 경험** — AI와 함께 앱을 처음부터 끝까지 만들어보는 경험을 해보고 싶었다.
...
- **결국 데이터가 핵심이다** — 아무리 잘 만들어도 네이버·카카오맵이 이미 더 많은 데이터와 리뷰를 갖고 있다. 외부 API에 의존하는 한 기능의 차별화에는 한계가 있다.
- **프롬프트를 정확하게 써야 한다** — 모호하게 지시하면 엉뚱한 방향으로 구현된다. 원하는 결과를 얻으려면 요구사항을 구체적이고 명확하게 정의하는 능력이 필요하다.
- **개발 실력보다 기획 능력이 더 중요해졌다** — AI가 코드를 짜주는 시대에는 "무엇을 만들지"를 정확히 설계하는 기획력이 핵심 역량이다. 코드를 직접 작성하는 능력보다, 문제를 정의하고 구조를 설계하는 능력이 결과물의 품질을 좌우한다.
